# Phase 4b: Adaptive Depth Loop - Solana

> **Usage**: Orchestrator reads this file and runs the Adaptive Depth Loop for Solana audits.
...
| Agent | Solana Focus |
|-------|-------------|
| depth-token-flow | Vault token account balance, unsolicited SPL/SOL transfers, Token-2022 extensions |
| depth-state-trace | Cross-account state, PDA invariants, CPI state mutations, instruction ordering |
| depth-edge-case | Zero-state, account initialization ordering, CU boundary testing, rent-exempt thresholds |
| depth-external | CPI side effects, program upgrade risk, instruction introspection, external program behavior |
...
ADAPTIVE_DEPTH_LOOP(findings_inventory):
  ...
  // â•â•â• INVARIANT FUZZ CAMPAIGN (Solana/Trident) â•â•â•
  ...
  // If trident_available == false â†’ skip (proptest fallback handled in Phase 5)
  // NOTE (V2): the fuzz campaign runs as a driver-scheduled depth fuzz sidecar
  // worker, not a coordinator spawn. See prompts/solana/v2/phase4b-invariant-fuzz.md.
  // The driver always emits the invariant worker in Thorough; the worker
  // self-selects Trident (primary) vs proptest/boundary (fallback) from
  // trident_available in build_status.md, and degrade-continues without halting
  // depth. Do NOT spawn an agent from here.
  ...
  if MODE == THOROUGH:
    spawn checklist_agent(model="haiku", prompt="
      ...
      Depth Agent -> Skill Mapping:
        depth-token-flow -> ~/.claude/agents/skills/solana/token-flow-tracing/SKILL.md
        depth-state-trace -> ~/.claude/agents/skills/solana/pda-security/SKILL.md
        depth-edge-case -> ~/.claude/agents/skills/solana/zero-state-return/SKILL.md
        depth-external -> ~/.claude/agents/skills/solana/external-precondition-audit/SKILL.md
  ...
  DONE:
    ...
    if len(covered) > 0 AND depth_spawns_used < max_depth_spawns:
      spawn resweep_agent(model="sonnet", prompt="
        ...
        These instructions already have findings. For each, the KNOWN topic is listed.
        ...
        2. What OTHER vulnerability classes could affect this function?
           ...
           side effects, account constraint violations, return value mishandling)
...
You are the {TYPE} Devil's Advocate Depth Agent - ITERATION {N} (targeted pass) for a Solana program audit.
...
### Finding [XX-N]: {Title}
- **Location**: {program.rs:L45-L67}
- **Evidence**: {[CODE] - relevant code snippet at L45}
- **Confidence**: {0.35}
- **Evidence Gap**: {What specific evidence is missing}
- **Prior Path**: {1-2 sentence summary of what the previous agent explored and how - NOT what it concluded. E.g., "Traced CPI state reload path; did not explore bump seed collision or PDA derivation variant."}
- **Investigate**: {Focused question - e.g., 'Can set_max_validators() be called with value below current validator count? Trace what happens to the loop at L120.'}
...
1. **Boundary Substitution**: Substitute boundary values into expressions. Tag: `[BOUNDARY:X=val â†’ outcome]`
   Solana examples: `[BOUNDARY:lamports=0 â†’ account garbage collected]`, `[BOUNDARY:CU=200000 â†’ tx fails]`
2. **Parameter Variation**: Vary inputs across valid range. Tag: `[VARIATION:param Aâ†’B â†’ outcome]`
   Solana examples: `[VARIATION:bump 255â†’254 â†’ different PDA address]`, `[VARIATION:mint Tokenâ†’Token-2022 â†’ transfer fee applied]`
3. **Trace to Termination**: Follow execution to terminal state. When a boundary value produces weight=0, contribution=0, or amount=0 in a computation, trace whether the zero-value entry still INCREMENTS a counter or PASSES a gate that downstream code relies on for correctness. **Nested call resolution**: When tracing through a CPI, also trace what happens when control returns to the caller - does it reload account data and verify post-CPI state? Tag: `[TRACE:pathâ†’outcome at L{N}]`
