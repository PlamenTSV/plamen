...
> `prompts/sui/v2/phase1-recon-prompt.md`; recon receives no MCP and external
...
knowledge of every external package call. depth-phase workers run with
...
- **Real Behavior**: researched real semantics (Tavily/web — official docs,
  verified package source, audit reports).
...
Return: 'DONE: design_context.md, external_production_behavior.md, external_dependency_research.md written. Fork ancestry: {found/none}. External packages: {N} verified, {M} unverified'
")
```
...
and misses every custom/unfamous external package. Every dependency that sets
