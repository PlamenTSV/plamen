# Phase 4a: Inventory Agent Prompt Template - Soroban

> **Usage**: Orchestrator reads this file and spawns the Inventory Agent for a Soroban contract audit.
> Replace placeholders `{SCRATCHPAD}`, `{list...}`, etc.
> **Includes**: Cross-contract call side effect trace audit (merged to eliminate a sequential gate).
...
You are the Inventory Agent for a Soroban contract audit.
...
| `set-contract-storage` | **Critical** | Unprotected contract storage write | Verify: is there any `require_auth` before the storage write? If yes → downgrade to High and flag for auth-trace depth |
| `unprotected-update-current-contract-wasm` | **Critical** | Unprotected WASM upgrade path | Any callable path to `update_current_contract_wasm` without timelock/governance → Critical |
...
| `missing-auth` | **High** | Missing authentication on privileged operation | If the function is callable by any address without `require_auth` → High minimum |
| `ttl-not-extended` | **High** | Persistent storage entry TTL not extended before access | Verify the entry could realistically expire; if TTL extension is in the same transaction elsewhere → Medium |
...
- RUSTSEC severity Critical/High + affects contract execution path → promote as [AUDIT-N] at the advisory severity
- RUSTSEC severity Medium + affects contract execution path → promote as [AUDIT-N] at Low (requires depth verification)
- RUSTSEC severity Low or affects only dev/test dependencies → skip promotion, note in static_analysis.md
...
- Storage is re-read after the call and result is validated → SAFE
- Storage is used stale after the cross-contract call → **FINDING** (SB6)
- External contract address is user-supplied without validation → **FINDING** (SB5)
- try_invoke_contract Result is silently discarded → **FINDING** (SB5)
- Auth is forwarded in a way the callee can exploit beyond original scope → **FINDING** (SB10)
...
For each storage key written by function A and read/depended-on by function B (where A ≠ B and A is externally callable):
