# Phase 4b: Adaptive Depth Loop - Soroban

> **Usage**: Orchestrator reads this file and runs the Adaptive Depth Loop for Soroban audits.
> Iteration 1 spawns ALL 8 agents (4 depth + 3 blind spot + 1 validation sweep) + niche agents. Iterations 2-3 are targeted and autonomous.
...
## Depth Agent Domain Mapping - Soroban

| Agent | Soroban Focus |
|-------|--------------|
| depth-token-flow | SEP-41 token interface, transfer/transfer_from/approve flows, SAC interaction, allowance expiry, balance accounting |
| depth-state-trace | Storage type correctness (Instance/Persistent/Temporary), TTL management, cross-function state mutation, require_auth coupling |
| depth-edge-case | Overflow (especially if overflow-checks=false), i128 boundary values, first depositor, empty state, TTL=0, instance storage near 64KB, archival edge cases |
| depth-external | Cross-contract calls (invoke_contract/try_invoke_contract), stale contractimport assumptions, oracle integration, bridge interaction, auth forwarding |
...
ADAPTIVE_DEPTH_LOOP(findings_inventory):
  ...
  // â•â•â• CARGO-FUZZ CAMPAIGN (Soroban - conditional) â•â•â•
  // Trigger: semantic_invariants.md exists AND nightly Rust toolchain available
  // Detection: recon sets CARGO_FUZZ_AVAILABLE in build_status.md
  //   (checked via: `rustup toolchain list | grep nightly`)
  // If CARGO_FUZZ_AVAILABLE == true:
  //   Read template: ~/.claude/prompts/soroban/phase4b-invariant-fuzz.md
  //   Spawn fuzz agent, 5-min timeout built into template
  //   Uses: `cargo +nightly fuzz run fuzz_target_1 -- -max_total_time=300`
  // If CARGO_FUZZ_AVAILABLE == false:
  //   Fallback: proptest-based boundary-value parameterized tests in Phase 5
  ...
  //   depth-token-flow: opus   (highest reasoning demand - SEP-41 invariants, SAC vs custom token)
  //   depth-state-trace: opus  (highest reasoning demand - storage types, TTL, auth coupling)
  //   depth-edge-case: sonnet  (pattern-matching - overflow, boundary values, TTL=0, first depositor)
  //   depth-external: sonnet   (pattern-matching - invoke_contract, oracle, bridge, auth forwarding)
  ...
  //   depth-token-flow: findings tagged token-flow, balance, transfer, SEP-41, SAC, allowance
  //   depth-state-trace: findings tagged state-trace, storage, TTL, require_auth, invariant, sync
  //   depth-edge-case: findings tagged edge-case, boundary, overflow, zero-state, archival, TTL=0
  ...
  if MODE == THOROUGH AND depth_spawns_used < max_depth_spawns:
    spawn perturbation_agent(model="sonnet", prompt="
      ...
      Step 2: Apply ALL applicable perturbation operators:
        | Operator | Action |
        |----------|--------|
        | DIRECTION_FLIP | Finding about op X â†’ read inverse op, check same class |
        | BOUNDARY_SHIFT | Finding at value V â†’ check region between 0 and V |
        | CONDITION_NEGATE | Missing check for C â†’ also check !C and related conditions |
        | OPERAND_SWAP | A op B â†’ check B op A (different rounding/precision) |
        | TEMPORAL_INVERT | Pre-action state bug â†’ check post-action state |

      Step 3: For each probe, read the actual source code and determine:
        - Is the perturbation a real vulnerability? (YES â†’ write finding)
        - Is there a defense? (YES â†’ note briefly, move on)
  ...
  if MODE == THOROUGH:
    spawn checklist_agent(model="haiku", prompt="
      ...
      Depth Agent â†’ Skill Mapping:
        depth-token-flow â†’ ~/.claude/agents/skills/soroban/token-flow-tracing/SKILL.md
        depth-state-trace â†’ ~/.claude/agents/skills/soroban/storage-lifecycle/SKILL.md
        depth-edge-case â†’ ~/.claude/agents/skills/soroban/overflow-safety/SKILL.md
        depth-external â†’ ~/.claude/agents/skills/soroban/external-precondition-audit/SKILL.md
    ...
    if len(covered) > 0 AND depth_spawns_used < max_depth_spawns:
      spawn resweep_agent(model="sonnet", prompt="
        ...
        2. What OTHER vulnerability classes could affect this function?
           (storage type mismatch, TTL expiry, auth bypass, cross-contract call safety,
           overflow in unchecked build, return value mishandling from try_invoke_contract)
...
You are the Confidence Scoring Agent (Batch: {DOMAIN}). You compute confidence scores for a batch of Soroban audit findings using a fixed formula.
...
You are the {TYPE} Devil's Advocate Depth Agent - ITERATION {N} (targeted pass) for a Soroban contract audit.
...
- Make your OWN tool calls. Produce NEW evidence that was not in iteration 1.
...
### Finding [XX-N]: {Title}
- **Location**: {contract.rs:L45-L67}
- **Evidence**: {[CODE] - relevant code snippet at L45}
- **Confidence**: {0.35}
- **Evidence Gap**: {What specific evidence is missing}
- **Prior Path**: {1-2 sentence summary of what the previous agent explored and how - NOT what it concluded. E.g., "Traced allowance decrement path through transfer_from; did not explore TTL expiry on the allowance entry or SAC vs custom token divergence."}
- **Investigate**: {Focused question - e.g., 'Can set_max_supply() be called with value below current total_supply? Trace what happens to the mint loop at L120.'}
