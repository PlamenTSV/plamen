# Phase 4b: Depth Agent Prompt Templates
...
ADAPTIVE_DEPTH_LOOP(findings_inventory):
  ...
  iter1_fixed = 10 + niche_injectable_count + 1  // 10 base (4 depth + 3 scanner + 1 VS + 1 SP + 1 DST) + niche
  ...
  // â•â•â• INVARIANT FUZZ CAMPAIGN (EVM only, MANDATORY in Thorough) â•â•â•
  ...
  // â•â•â• MEDUSA FUZZ CAMPAIGN (EVM Thorough only, MANDATORY if installed) â•â•â•
  ...
  //   Blind Spot Scanner A: sonnet (token coverage + parameter checks are checklist-driven), Scanner B: sonnet, Scanner C: sonnet
  ...
  for agent in completed_agents:
    if agent.timed_out:
      ...
      spawn depth-{agent.domain}-lite(lite_a, no_slither=true, max_files=5)
      spawn depth-{agent.domain}-lite(lite_b, no_slither=true, max_files=5)
  ...
  if MODE == THOROUGH AND len(niche_agents) > 0:
    ...
    // If gaps found, re-prompt specific niche agents with ONLY the missed entities
  ...
  // â•â•â• ITERATION 2: Micro-Niche Targeted Depth (Thorough only) â•â•â•
  ...
  DONE:
    ...
    // Deduplicate by function name. For each, record the known finding topic.
    if len(covered) > 0 AND depth_spawns_used < max_depth_spawns:
      spawn resweep_agent(model="sonnet", prompt="
        ...
        Analyze each function for vulnerability classes OUTSIDE the known topic.
        ...
        2. What OTHER vulnerability classes could affect this function?
           ...
           side effects, caller assumption violations, return value mishandling)
      ...
      depth_spawns_used += 1
      await resweep_agent
      // If resweep found new findings, they feed into chain analysis via findings_inventory
    ...
    spawn variable_map_agent(model="sonnet", prompt="
      ...
      (e.g., 'stale when pendingYield==0', 'diverges from sharePrice after recordLoss').
...
2. For each UNCERTAIN finding, generate a targeted manual investigation question based on:
   - Fork ancestry patterns from `meta_buffer.md` (e.g., "Yearn V2 locked profit: is loss branch complete?")
...
### Finding [XX-N]: {Title}
- **Location**: {Contract.sol:L45-L67}
- **Evidence**: {[CODE] - relevant code snippet at L45}
- **Confidence**: {0.35}
- **Evidence Gap**: {What specific evidence is missing}
- **Prior Path**: {1-2 sentence summary of what the previous agent explored and how - NOT what it concluded. E.g., "Traced numerator manipulation via supply inflation; did not explore divisor staleness."}
...
3. **Trace to Termination**: Follow execution to terminal state. When a boundary value produces weight=0, contribution=0, or amount=0 in a computation, trace whether the zero-value entry still INCREMENTS a counter or PASSES a gate that downstream code relies on for correctness. Tag: `[TRACE:pathâ†’outcome at L{N}]`
...
2. Make your OWN MCP tool calls:
   - get_function_source() for the relevant functions
