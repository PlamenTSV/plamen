...
Status: CONTRACT_ONLY_PENDING_INDEPENDENT_REVIEW successor; inherited v1 text below has legacy-only authority
...
## Program Facts public-v3 shadow amendment

Status: CONTRACT_ONLY_PENDING_INDEPENDENT_REVIEW. This amendment grants no
runtime, provider, consumer, finding, publication, package, or cutover authority.

The only public logical identities added by this successor are, in order,
`mechanical_program_facts.v3.json`, `mechanical_program_facts_receipt.v3.json`,
and `mechanical_program_facts_debt.v3.json`. They are generation-indirected
logical names selected solely by an ArtifactLedger active-head record. The v1
triple remains read-only legacy input; experimental v2 is never selected or
migrated in place. Compatibility dispatch is the closed set `ACTIVE_V3`,
`LEGACY_V1`, `NO_SIDECAR`, and `INTEGRITY_BLOCKED`; SHADOW audit semantics always
use the legacy/no-sidecar branch. `ACTIVE.v1.json` is a repairable projection,
never selection authority.

The six closed public contracts are the exact candidates at
...
`rules/schemas/program_facts_active_selection.v1.schema.json`. All facts are
additive proposals, all terminal-negative authority is false, and all nine
consumer/finding/severity/confidence/phase/clean-certification/refutation/
suppression/terminal-negative authority bits are false.

The sole runtime/stage/identity owner is the accepted Program Facts runtime
specification. PhaseIO is the only control/public byte writer, WorkerTransaction
writes only its armed PF-40 staging, and ArtifactLedger alone commits the active
head. The actor vocabulary remains `MODEL|DRIVER`; every Program Facts artifact
uses `DRIVER`. Exclusive publication order is `MATERIALIZE_IMMUTABLE_V3`, then
`COMMIT_ACTIVE_HEAD_CAS_V1`, then `MATERIALIZE_ACTIVE_PROJECTION_V1`. Lock order
is audit-run, program-facts-generation, PhaseIO work-unit, WTx children sorted by
expected-child ID, ArtifactLedger CAS, with reverse release.

The mandatory lifecycle is exactly G3-00 through G3-12 in ascending numeric
order. The feature remains absent/default false until an independently reviewed
postimplementation release freeze, soak evidence, and final cutover receipt all
exist. Non-EVM rows are typed no-launch/debt only in Gate 3.
