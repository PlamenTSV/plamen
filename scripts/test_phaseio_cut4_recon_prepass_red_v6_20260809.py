"""V6 RED denominator for the live Cut-4 recon prepass boundary.
...
V4_HASHES = {
    "test_phaseio_cut4_recon_prepass_red_v4_20260809.py":
        "5f92e515f7e48ee482d5e97d5dd46bcd312efe458a5903a976a13e69cdd120fb",
    "test_phaseio_cut4_recon_dependency_merge_red_v4_20260809.py":
        "aebf1982162b610a73c899ccfb56e59fcf5da880db34e86f53c50aa369e36264",
}
V5_HASHES = {
    ...
    "test_phaseio_cut4_recon_dependency_merge_red_v5_20260809.py":
        "8e1d47664bb0534de962c67f347bc70d06f18bd9f5dd489536b7ac96773ad912",
}
...
def _owns_committed_expected_output(
    ...
    if not all(
        ...
        for value in (artifacts, bindings, old_artifacts, expected_records)
    ) or type(recorded) not in {list, tuple} or not recorded:
        return False
    if not all(
        ...
        return False
    expected_identities = {
        identity for identity in old_artifacts
        if isinstance(identity, str) and identity.startswith("scratchpad:")
    }
    owned = expected_identities & set(artifacts)
    if not owned or len(recorded) != len(set(recorded)) or not owned.issubset(recorded):
        return False
...
def _commit_fixture_prepass(root: Path, project: Path) -> None:
    ...
    contract = PhaseIOContract(
        ...
        outputs=tuple(
            ArtifactSpec(
                ...
                writer="DRIVER",
                write_mode="CREATE",
                minimum_gate="V6_FALSE_GREEN_CONTROL",
            )
    ...
    for name in SC_PREPASS:
        (root / name).write_text(
            f"# {name}\n\nfirst committed V6 control generation\n" + "x" * 160 + "\n",
            encoding="utf-8",
        )
...
def _registered_successor_from_committed_readback(
    ...
    """Rebind one real transaction record to the exact future attempt shape."""
    assert AL.read_artifact_ledger(root) == before
    old = before["work_units"][old_key]
    artifact_identity = sorted(old["artifacts"])[0]
    source_record = old["artifacts"][artifact_identity]
    source_binding = before["artifact_bindings"][artifact_identity]
    raw = (root / artifact_identity.removeprefix("scratchpad:")).read_bytes()
    ...
    successor = {
        **expected_attempt,
        ...
        "artifacts": {artifact_identity: record},
    }
    successor["commit_authority"] = {
        **expected_attempt,
        ...
        "reason_codes": [],
        "recorded_output_identities": [artifact_identity],
        "expected_output_records": {
            artifact_identity: {"sha256": digest, "size": len(raw)}
        },
    }
    after["work_units"][producer] = successor
    after["artifact_bindings"][artifact_identity] = binding
    return after, artifact_identity


def test_v1_v2_v3_v4_v5_fixture_preimages_remain_byte_frozen() -> None:
...
def test_v6_accepts_exact_registered_prepass_successor_with_real_output_control(
    tmp_path: Path,
) -> None:
    project, scratchpad, config = _workspace(
        tmp_path, pipeline="sc", mode="core", route="codex"
    ...
    after, _artifact_identity = _registered_successor_from_committed_readback(
...
@pytest.mark.parametrize("fault", ("zero_artifacts", "unrelated_owner", "bad_bytes"))
def test_v6_rejects_uncommitted_prepass_registered_successor(
    tmp_path: Path,
    fault: str,
) -> None:
    project, scratchpad, config = _workspace(
    ...
    after, artifact_identity = _registered_successor_from_committed_readback(
        scratchpad, before, old_key, expected_attempt
    )
    producer = expected_attempt["producer_attempt_identity"]
    if fault == "zero_artifacts":
        after["work_units"][producer]["artifacts"] = {}
    elif fault == "unrelated_owner":
        after["work_units"][producer]["artifacts"][artifact_identity][
            "owner_key"
        ] = old_key
        after["artifact_bindings"][artifact_identity]["owner_key"] = old_key
    else:
        path = scratchpad / artifact_identity.removeprefix("scratchpad:")
