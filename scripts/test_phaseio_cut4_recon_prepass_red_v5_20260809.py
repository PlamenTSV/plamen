"""V5 RED denominator for the live Cut-4 recon prepass boundary.
    ...
    *,
    ...
    def _bound_disposition(row: Mapping[str, Any]) -> bool:
        ...
        return (
            ...
            and disposition.get("state") in {"QUARANTINED", "DEBT", "REJECTED", "FAILED"}
            and isinstance(disposition.get("reason_codes"), list)
            and bool(disposition.get("reason_codes"))
...
def _commit_fixture_prepass(root: Path, project: Path) -> None:
    ...
    contract = PhaseIOContract(
        ...
        outputs=tuple(
            ArtifactSpec(
                ...
                writer="DRIVER",
                write_mode="CREATE",
                minimum_gate="V5_FALSE_GREEN_CONTROL",
            )
    ...
    for name in SC_PREPASS:
        (root / name).write_text(
            f"# {name}\n\nfirst committed V5 control generation\n" + "x" * 160 + "\n",
            encoding="utf-8",
        )
...
def test_v1_v2_v3_v4_fixture_preimages_remain_byte_frozen() -> None:
...
@pytest.mark.parametrize("mutation", ("source_edit", "config_edit", "unexpected_output"))
def test_v5_rejects_v3_prepass_second_call_noop_counterexample(
    tmp_path: Path,
    mutation: str,
) -> None:
    project, scratchpad, config = _workspace(
...
def test_v5_rejects_v4_prepass_invalidation_or_deletion_only_counterexample(
    tmp_path: Path,
    transition: str,
) -> None:
    project, scratchpad, config = _workspace(
...
def test_v5_accepts_exact_registered_prepass_successor_attempt_control(
    tmp_path: Path,
) -> None:
    project, scratchpad, config = _workspace(
        tmp_path, pipeline="sc", mode="core", route="codex"
    ...
    successor["commit_authority"] = {
        **expected_attempt,
        ...
        "state": "ACTIVE",
        "work_unit_key": identity,
        "authority_digest": expected_attempt["attempted_authority_digest"],
        "reason_codes": [],
    }
...
def test_real_second_driver_invocation_uses_completed_recon_resume_branch(
    ...
    run_root, _project, scratchpad, config_path, call_log = DRIVER_SMOKE._make_project(
        f"cut4_v5_resume_{resume_branch}_", mode="light", pipeline="sc",
