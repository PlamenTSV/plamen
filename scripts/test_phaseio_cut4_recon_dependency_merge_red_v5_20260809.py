"""V5 live-entry RED denominator for recon fanout, dependency, and merge.
...
@pytest.mark.parametrize("drift", ("shard_bytes", "source_bytes"))
def test_v5_rejects_v3_canonical_second_call_noop_counterexample(
    tmp_path: Path,
    drift: str,
) -> None:
    project, scratchpad, config = _workspace(
...
def test_v5_rejects_v4_canonical_unbound_debt_false_green(
    tmp_path: Path,
) -> None:
    _scratchpad, _config, before, old_key, _old, expected_attempt = (
...
def test_v5_accepts_exact_bound_canonical_debt_failed_attempt_control(
    tmp_path: Path,
) -> None:
    _scratchpad, _config, before, old_key, _old, expected_attempt = (
        _canonical_control_preimage(tmp_path)
